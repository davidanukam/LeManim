...
class Intro(Scene):
    def construct(self):
        ...
        # NOTE: Create the question marks
        question_mark_left = (
            Text("?", font_size=50)
            .scale(2)
            .shift(LEFT * 1.5)
        ...
        self.wait()
        lightbulb = SVGMobject("assets/Lightbulb.svg").move_to(brain)
        self.play(ReplacementTransform(brain, lightbulb), run_time=1)

        # NOTE: Set up screen for next scene
        self.wait(0.5)
        self.play(
            FadeOut(lightbulb),
            x_plus_y.animate.to_edge(UP),
            list_of_nums.animate.shift(DOWN * 0.5),
        )
